...
const ESPHOME_TEMPLATE_SNIPPETS = {
  wifi_ota: `wifi:
...
const ESPHOME_TEMPLATE_HEADER = `# ESPHome configuration for Seeed reTerminal E-Series
...
logger:
  level: INFO`;
const ESPHOME_TEMPLATE_FOOTER = `# secrets.yaml (create this file alongside your config):
...
// 根据已勾选的模板选项生成 ESPHome YAML 文件内容。
function buildEsphomeTemplateYaml(selectedOptionIds) {
  const snippets = selectedOptionIds
    .map((optionId) => ESPHOME_TEMPLATE_SNIPPETS[optionId])
    .filter(Boolean);

  return [
    ESPHOME_TEMPLATE_HEADER,
    ...snippets,
    ESPHOME_TEMPLATE_FOOTER,
  ].join("\n\n") + "\n";
}

// Downloads generated text content as a local file.
// 将生成的文本内容下载成本地文件。
...
// Collects template checkbox state and exports the ESPHome YAML file.
// 收集模板复选框状态，并导出 ESPHome YAML 文件。
function generateEsphomeYaml() {
  if (!selectedPlatform?.templateMode || !selectedDevice) return;

  const selectedOptionIds = (selectedPlatform.templateOptions || [])
    .filter((option) => {
      const input = document.getElementById(option.id);
      return input?.checked;
    })
    .map((option) => option.id);
  const yaml = buildEsphomeTemplateYaml(selectedOptionIds);
  downloadTextFile(`esphome-reterminal-${selectedDevice.id}.yaml`, yaml, "text/yaml");
  appendLog(`[system] Generated ESPHome YAML for ${selectedDevice.id}.`);
}
...
function bindWorkspaceEvents() {
  ...
  if (generateYamlBtn) {
    generateYamlBtn.addEventListener("click", generateEsphomeYaml);
  }
}
