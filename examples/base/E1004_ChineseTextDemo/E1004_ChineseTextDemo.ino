 ...
 *
 * Minimal Chinese text rendering test for reTerminal E1004.
 *
...
static void drawChineseDemo()
{
  ...
  display.fillSprite(TFT_WHITE);
  display.drawRoundRect(margin - 28, 88, w - (margin - 28) * 2, h - 176,
                        18, TFT_BLACK);

  render.setFontColor(TFT_BLACK, TFT_WHITE);

  render.setFontSize(66);
  render.setCursor(margin, 196);
  render.printf("中文显示测试");

  render.setFontSize(52);
  render.setCursor(margin, 336);
  render.printf("你好，世界。");
  ...
  render.setCursor(margin, 596);
  render.printf("混合文本：E1004 + OpenFontRender + SPIFFS");

  render.setFontSize(34);
  render.setCursor(margin, 720);
...
void setup()
{
  ...
  LOG.println("=========================================");
  LOG.println("  E1004 Chinese Text Demo");
  LOG.println("=========================================");
