# E1004 Chinese Text Demo

这个示例只验证一件事：reTerminal E1004 能不能从 Flash 文件系统加载 TTF 字体，并把 UTF-8 中文画到 1200x1600 竖屏电子纸屏幕上。
...
| File | Purpose |
| --- | --- |
| `E1004_ChineseTextDemo.ino` | Arduino 入口程序。初始化屏幕、挂载 SPIFFS、加载字体、绘制中文。 |
| `driver.h` | 固定 `BOARD_SCREEN_COMBO 523`，让 Seeed_GFX 使用 reTerminal E1004 屏幕驱动。 |
| `partitions.csv` | 本示例专用 8 MB Flash 分区表，给 SPIFFS 预留约 4.9 MB。 |
| `data/font_cn.ttf` | 中文 TTF 字体，上传到 SPIFFS 后运行时通过 `/font_cn.ttf` 加载。 |

## Panel Notes

- Panel: reTerminal E1004
- Controller: T133A01
...
1. 打开 `E1004_ChineseTextDemo.ino`。
...
```sh
mkdir -p /private/tmp/e1004-chinese-spiffs

/Users/mengdu/Library/Arduino15/packages/esp32/tools/mkspiffs/0.2.3/mkspiffs \
  -c /Users/mengdu/Desktop/OSHW-reTerminal-Series-E-D/examples/base/E1004_ChineseTextDemo/data \
  -p 256 \
  -b 4096 \
  -s 0x4E0000 \
  /private/tmp/e1004-chinese-spiffs/e1004_chinese.spiffs.bin
```
...
- `你好，世界。`
- `备忘录：明天下午三点取快递。`
- `混合文本：E1004 + OpenFontRender + SPIFFS`
