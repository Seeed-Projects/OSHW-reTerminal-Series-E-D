# E1003 Chinese Text Demo

这个示例只验证一件事：reTerminal E1003 能不能从 Flash 文件系统加载 TTF 字体，并把 UTF-8 中文画到电子纸屏幕上。
...
| File | Purpose |
| --- | --- |
| `E1003_ChineseTextDemo.ino` | Arduino 入口程序。初始化屏幕、挂载 SPIFFS、加载字体、绘制中文。 |
| `driver.h` | 固定 `BOARD_SCREEN_COMBO 522`，让 Seeed_GFX 使用 reTerminal E1003 屏幕驱动。 |
...
1. 打开 `E1003_ChineseTextDemo.ino`。
...
/Users/mengdu/Library/Arduino15/packages/esp32/tools/mkspiffs/0.2.3/mkspiffs \
  ...
  -b 4096 \
  -s 0x4E0000 \
  /private/tmp/e1003-chinese-spiffs/e1003_chinese.spiffs.bin
```
...
- `你好，世界。`
- `备忘录：明天下午三点取快递。`
- `混合文本：reTerminal E1003 + OpenFontRender + SPIFFS`
