 ...
 *
 * Minimal Chinese text rendering test for reTerminal E1003.
 *
...
static void drawBootMessage(const char* title, const char* body)
{
  ...
  display.drawString(title, 80, 80);

  display.setTextSize(3);
  display.drawString(body, 80, 160);
  display.update();
}
...
static void drawChineseDemo()
{
  ...
  display.fillSprite(TFT_WHITE);
  display.drawRoundRect(margin - 28, 64, w - (margin - 28) * 2, h - 128,
                        18, TFT_BLACK);

  render.setFontColor(TFT_BLACK, TFT_WHITE);

  render.setFontSize(72);
  render.setCursor(margin, 138);
  render.printf("中文显示测试");

  render.setFontSize(52);
  render.setCursor(margin, 260);
  render.printf("你好，世界。");

  render.setFontSize(44);
  render.setCursor(margin, 368);
  render.printf("备忘录：明天下午三点取快递。");

  render.setFontSize(38);
  render.setCursor(margin, 468);
  render.printf("混合文本：reTerminal E1003 + OpenFontRender + SPIFFS");

  render.setFontSize(34);
  render.setCursor(margin, 560);
  render.printf("如果这一页中文清晰可读，就可以继续做中文版备忘录。");

  display.setTextDatum(BL_DATUM);
  display.setTextSize(3);
  display.setTextColor(TFT_BLACK, TFT_WHITE, true);
  display.drawString("Font: /font_cn.ttf", margin, h - 96);
...
void setup()
{
  ...
  LOG.println("=========================================");
  LOG.println("  E1003 Chinese Text Demo");
  LOG.println("=========================================");
