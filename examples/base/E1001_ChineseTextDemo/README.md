# E1001 Chinese Text Demo

这个示例只验证一件事：reTerminal E1001 能不能从 Flash 文件系统加载 TTF 字体，并把 UTF-8 中文画到 800x480 电子纸屏幕上。
...
| File | Purpose |
| --- | --- |
| `E1001_ChineseTextDemo.ino` | Arduino 入口程序。初始化屏幕、挂载 SPIFFS、加载字体、绘制中文。 |
| `driver.h` | 固定 `BOARD_SCREEN_COMBO 520`，让 Seeed_GFX 使用 reTerminal E1001 屏幕驱动。 |
| `partitions.csv` | 本示例专用 8 MB Flash 分区表，给 SPIFFS 预留约 4.9 MB。 |
| `data/font_cn.ttf` | 中文 TTF 字体，上传到 SPIFFS 后运行时通过 `/font_cn.ttf` 加载。 |

## Panel Notes

- Panel: reTerminal E1001
- Controller: UC8179
- Resolution: 800x480 landscape
- Color mode: monochrome panel with 4-level grayscale via `initGrayMode(GRAY_LEVEL4)`
...
1. 打开 `E1001_ChineseTextDemo.ino`。
...
```sh
mkdir -p /private/tmp/e1001-chinese-spiffs

/Users/mengdu/Library/Arduino15/packages/esp32/tools/mkspiffs/0.2.3/mkspiffs \
  -c /Users/mengdu/Desktop/OSHW-reTerminal-Series-E-D/examples/base/E1001_ChineseTextDemo/data \
  -p 256 \
  -b 4096 \
  -s 0x4E0000 \
  /private/tmp/e1001-chinese-spiffs/e1001_chinese.spiffs.bin
```
